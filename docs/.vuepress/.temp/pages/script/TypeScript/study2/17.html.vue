<template><div><h1 id="十七、d-ts-类型声明文件" tabindex="-1"><a class="header-anchor" href="#十七、d-ts-类型声明文件"><span>十七、d.ts 类型声明文件</span></a></h1>
<h2 id="_17-1-简介" tabindex="-1"><a class="header-anchor" href="#_17-1-简介"><span>17.1 简介</span></a></h2>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_17-2-类型声明文件的来源" tabindex="-1"><a class="header-anchor" href="#_17-2-类型声明文件的来源"><span>17.2 类型声明文件的来源</span></a></h2>
<p>类型声明文件主要有以下三种来源。</p>
<ul>
...
<p>这些内置声明文件位于 TypeScript 语言安装目录的<code v-pre>lib</code>文件夹内，数量大概有几十个，下面是其中一些主要文件。</p>
<ul>
<li>lib.d.ts</li>
<li>lib.dom.d.ts</li>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上面示例中，<code v-pre>lib</code>选项指定加载<code v-pre>dom</code>和<code v-pre>es2021</code>这两个内置类型声明文件。</p>
<p>编译选项<code v-pre>noLib</code>会禁止加载任何内置声明文件。</p>
<h3 id="_17-2-3-外部类型声明文件" tabindex="-1"><a class="header-anchor" href="#_17-2-3-外部类型声明文件"><span>17.2.3 外部类型声明文件</span></a></h3>
<p>如果项目中使用了外部的某个第三方代码库，那么就需要这个库的类型声明文件。</p>
<p>这时又分成三种情况。</p>
<p>（1）这个库自带了类型声明文件。</p>
<p>一般来说，如果这个库的源码包含了<code v-pre>[vendor].d.ts</code>文件，那么就自带了类型声明文件。其中的<code v-pre>vendor</code>表示这个库的名字，比如<code v-pre>moment</code>这个库就自带<code v-pre>moment.d.ts</code>。使用这个库可能需要单独加载它的类型声明文件。</p>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上面示例是一个模块的 package.json 文件，该模块需要 browserify 模块。由于后者的类型声明文件是一个单独的模块<code v-pre>@types/browserify</code>，所以还需要加载那个模块。</p>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>编译器会在预处理阶段，找出所有三斜杠引用的文件，将其添加到编译列表中，然后一起编译。</p>
<p><code v-pre>path</code>参数指定了所引入文件的路径。如果该路径是一个相对路径，则基于当前脚本的路径进行计算。</p>
<p>使用该命令时，有以下两个注意事项。</p>
<ul>
<li><code v-pre>path</code>参数必须指向一个存在的文件，若文件不存在会报错。</li>
<li><code v-pre>path</code>参数不允许指向当前文件。</li>
</ul>
<p>默认情况下，每个三斜杠命令引入的脚本，都会编译成单独的 JS 文件。如果希望编译后只产出一个合并文件，可以使用编译选项<code v-pre>outFile</code>。但是，<code v-pre>outFile</code>编译选项不支持合并 CommonJS 模块和 ES 模块，只有当编译参数<code v-pre>module</code>的值设为 None、System 或 AMD 时，才能编译成一个文件。</p>
<p>如果打开了编译参数<code v-pre>noResolve</code>，则忽略三斜杠指令。将其当作一般的注释，原样保留在编译产物中。</p>
...
<p>注意，这个命令只在你自己手写类型声明文件（<code v-pre>.d.ts</code>文件）时，才有必要用到，也就是说，只应该用在<code v-pre>.d.ts</code>文件中，普通的<code v-pre>.ts</code>脚本文件不需要写这个命令。如果是普通的<code v-pre>.ts</code>脚本，可以使用<code v-pre>tsconfig.json</code>文件的<code v-pre>types</code>属性指定依赖的类型库。</p>
<h3 id="_17-5-3-reference-lib" tabindex="-1"><a class="header-anchor" href="#_17-5-3-reference-lib"><span>17.5.3 <code v-pre>/// &lt;reference lib=&quot;&quot; /&gt;</code></span></a></h3>
